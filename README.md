...
3. Нажми Play. Сцена полностью настроена — дополнительных действий не
   требуется.
4. Покупки в редакторе идут через встроенный в Unity IAP **Fake Store**
   (реальный аккаунт стора или продукт не нужны).
...
портретного телефона класса 1080x2340 с safe-area отступом).

Также был выполнен настоящий `Build > Android` с этого коммита, и он
**завершился успешно**: `Builds/Android/MiniFactory.apk`, ~73 МБ, IL2CPP,
ARM64 + ARMv7, ноль ошибок сборки (974 предупреждения, почти все — шум
шейдерных вариантов от `com.unity.ai.inference`/Sentis, см. «Дальнейшие
шаги»). Вся сборка заняла около 52 минут, большую часть времени — нативная
компиляция IL2CPP и первая закачка зависимостей Gradle.

При установке этого APK на реальный телефон обнаружился настоящий баг:
**кнопка «Купить монеты» не реагировала**. Причина — `Assets/Resources/
BillingMode.json` был выставлен в `{"androidStore":"GooglePlay"}`, то есть
в реальной сборке (в отличие от редактора) Unity IAP пытался подключиться
к настоящему Google Play Billing, для которого не зарегистрирован ни
продукт, ни правильный package name — инициализация IAP тихо проваливалась
без видимой реакции на экране. Исправлено на
`{"androidStore":"fake"}`, чтобы и в сборке использовался Fake Store, как
и предполагает ТЗ («для проверки достаточно использовать Fake Store»).
**Это исправление ещё не попало в собранный APK** — нужна новая сборка,
которая на данный момент отложена (см. «Дальнейшие шаги»).
...
примерно 4+ часа 16.09.2026.

**Что осталось / что бы я сделал следующим шагом:**

1. Пересобрать APK с исправленным `BillingMode.json` (`"fake"` вместо
   `"GooglePlay"`) и установить на реальное устройство, проверить, что
   покупка теперь реагирует и проходит через Fake Store.
2. Убрать `com.unity.ai.assistant` / `com.unity.ai.inference` (Sentis) —
   неиспользуемые остатки шаблона, на которые приходится почти всё 974
   предупреждения сборки и, вероятно, заметная часть ~52-минутного
   времени сборки и ~73 МБ размера APK.
3. Поменять Android package name с дефолтного шаблонного перед любым
   реальным распространением.
4. Выбрать и подключить один бонусный пункт — Firebase Remote Config
   выглядит самым полезным, учитывая, что точка расширения
   `IConfigProvider` для этого уже готова.
5. Добавить пару Play Mode тестов на мобильный lifecycle
