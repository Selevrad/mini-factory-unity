...
1. Поменять Android package name с дефолтного шаблонного перед любым
   реальным распространением.
2. Выбрать и подключить один бонусный пункт — Firebase Remote Config
   выглядит самым полезным.
3. Добавить пару Play Mode тестов на мобильный lifecycle
